...
class _FlowFakeRepository extends SimulationRepository {
  ...
  Stream<List<SimulationModel>> watchSimulations(String patientId) {
    return Stream.value(simulations.where((s) => s.patientId == patientId).toList());
  }
  ...
  Stream<SimulationModel?> watchSimulation({
    ...
    if (lastCreated != null && lastCreated!.patientId == patientId && lastCreated!.id == simulationId) {
      return Stream.value(lastCreated);
    }
    final existing = simulations.where((s) => s.patientId == patientId && s.id == simulationId);
  ...
  @override
  Future<String?> resolveMediaUrl(String? pathOrUrl) async => 'https://example.com/image.jpg';
}
...
void main() {
  test('después de pickOriginalFromCamera el estado queda draftReady', () async {
    final repo = _FlowFakeRepository();
    final container = ProviderContainer(
      overrides: [
        simulationRepositoryProvider.overrideWith((ref) => repo),
        imagePickerServiceProvider.overrideWith((ref) => _FakePickerService()),
        faceDetectionServiceProvider.overrideWith((ref) => _FakeFaceDetectionService()),
      ],
    );
    addTearDown(container.dispose);

    await container.read(simulatorFlowProvider.notifier).pickOriginalFromCamera(
      patientId: 'p1',
      adminId: 'admin-1',
      treatmentType: TreatmentType.convencional,
    );
  ...
  testWidgets('si falta API key se muestra mensaje claro al intentar generar', (tester) async {
    await tester.binding.setSurfaceSize(const Size(800, 1400));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    final repo = _FlowFakeRepository()
      ..generateError = 'El simulador IA está instalado, pero falta configurar la API KEY en Firebase Functions.';
    final flow = SimulatorFlowState(
      uiState: SimulatorUiState.draftReady,
      patientId: 'p1',
      simulationId: 'sim-draft',
    ...
    await tester.pumpWidget(
      _wrap(
        ...
        overrides: [
          simulationRepositoryProvider.overrideWith((ref) => repo),
          simulatorFlowProvider.overrideWith(() => _MutableFlowNotifier(flow, repo)),
        ],
      ),
    );
    await tester.pumpAndSettle();
    ...
    expect(
      find.text('El simulador IA está instalado, pero falta configurar la API KEY en Firebase Functions.'),
      findsOneWidget,
    );
  });

  testWidgets('con borrador activo el flujo activo se muestra antes del historial', (tester) async {
    ...
    final sims = [
      _sim(id: 'sim-old', status: SimulationStatus.ready, resultPath: 'simulations/p1/sim-old/result.jpg'),
      _sim(id: 'sim-older', status: SimulationStatus.shared, shared: true, resultPath: 'simulations/p1/sim-older/result.jpg'),
    ];
    ...
    await tester.pumpWidget(
      _wrap(
        ...
        overrides: [
          authStateProvider.overrideWith((ref) => Stream.value(_FakeUser('admin-1'))),
          simulationRepositoryProvider.overrideWith((ref) => repo),
          patientSimulationsProvider('p1').overrideWith((ref) => Stream.value(sims)),
          simulatorFlowProvider.overrideWith(() => _StaticFlowNotifier(activeFlow)),
        ],
      ),
    );
    await tester.pumpAndSettle();

    final activeFlowFinder = find.byKey(const ValueKey('simulator-active-flow'));
    final historyFinder = find.byKey(const ValueKey('simulation-history-title'));

    expect(activeFlowFinder, findsOneWidget);
    expect(historyFinder, findsOneWidget);
    expect(tester.getTopLeft(activeFlowFinder).dy, lessThan(tester.getTopLeft(historyFinder).dy));
  });
}
...
class _MutableFlowNotifier extends SimulatorFlowNotifier {
  ...
  @override
  Future<void> generateWithAi({required String patientId, required String treatmentType}) async {
    ...
    try {
      ...
      state = AsyncData(
        current.copyWith(
          ...
          errorMessage: 'El simulador IA está instalado, pero falta configurar la API KEY en Firebase Functions.',
        ),
      );
    }
  }
