...
void main() {
  group('PatientTreatmentsRepository', () {
    ...
    test(
      'guarda tratamiento principal y sincroniza paciente y payments',
      () async {
        ...
        final treatmentDoc = await db
            .collection('patients/patient-1/treatments')
            .doc('tx-1')
            .get();
    ...
    test('cambia tratamiento principal y desmarca el anterior', () async {
      ...
      final firstDoc = await db
          .collection('patients/patient-1/treatments')
          .doc('tx-1')
          .get();
      final secondDoc = await db
          .collection('patients/patient-1/treatments')
          .doc('tx-2')
          .get();
    ...
    test(
      'migra paciente viejo con tratamiento plano a subcolección sin perder datos',
      () async {
        ...
        expect(
          patientDoc.data()?['primaryTreatmentId'],
          'migrated-primary-patient-legacy',
        );
    ...
    test(
      'permite coexistencia de segundo y tercer tratamiento sin sobrescribir',
      () async {
        final first = PatientTreatment(
          id: 'tx-1',
          patientId: 'patient-1',
        ...
        expect(
          snapshot.docs.firstWhere((doc) => doc.id == 'tx-1').data()['name'],
          'Convencional',
        );
        expect(
          snapshot.docs.firstWhere((doc) => doc.id == 'tx-2').data()['name'],
          'Alineadores',
        );
        expect(
          snapshot.docs.firstWhere((doc) => doc.id == 'tx-3').data()['name'],
          'Retenedores',
        );
        expect(
          snapshot.docs
              .firstWhere((doc) => doc.id == 'tx-1')
              .data()['isPrimary'],
          isTrue,
        );
        expect(
          snapshot.docs
              .firstWhere((doc) => doc.id == 'tx-2')
              .data()['isPrimary'],
          isFalse,
        );
        expect(
          snapshot.docs
              .firstWhere((doc) => doc.id == 'tx-3')
              .data()['isPrimary'],
          isFalse,
        );
      },
    ...
    test(
      'no remigra si el paciente viejo ya tiene tratamiento estructurado',
      () async {
        final patient = PatientModel(
          id: 'patient-legacy-2',
        ...
        await db
            .collection('patients/patient-legacy-2/treatments')
            .doc('tx-existing')
            .set({
              'id': 'tx-existing',
    ...
    test('getPatientTreatments conserva el id del documento', () async {
      await db
          .collection('patients/patient-1/treatments')
          .doc('tx-doc-id')
          .set({
      ...
      final treatments = await repo.getPatientTreatments('patient-1');

      expect(treatments.single.id, 'tx-doc-id');
      expect(treatments.single.etapaActual, TreatmentStage.controles);
    });
  });
}
