...
final clinicalFilesRepositoryProvider = Provider<ClinicalFilesRepository>((ref) {
  return ClinicalFilesRepository(FirebaseFirestore.instance);
});

final clinicalFilesStorageProvider = Provider<ClinicalFilesStorageService>((ref) {
...
final patientClinicalFilesProvider =
    StreamProvider.family<List<ClinicalFileModel>, ({String patientId, String? treatmentId, bool onlyVisibleToPatient})>((ref, args) {
  return ref.watch(clinicalFilesRepositoryProvider).watchFiles(
        args.patientId,
...
class UploadClinicalFileNotifier extends AsyncNotifier<void> {
  ...
  Future<void> upload({
    ...
    state = await AsyncValue.guard(() async {
      ...
      final url = await ref.read(clinicalFilesStorageProvider).upload(
            patientId: patientId,
      ...
      final model = ClinicalFileModel(
        ...
        displayName: displayName.trim().isEmpty ? picked.fileName : displayName.trim(),
        storagePath: '',
        downloadUrl: url,
      ...
      await ref.read(clinicalFilesRepositoryProvider).saveMetadata(model.copyWith(
            storagePath: StoragePaths.patientClinicalFile(
              patientId,
              model.id,
              picked.fileName,
  ...
  Future<void> softDelete({
    ...
    state = const AsyncLoading();
    state = await AsyncValue.guard(() => ref.read(clinicalFilesRepositoryProvider).softDelete(
          patientId: patientId,
          fileId: fileId,
          deletedBy: deletedBy,
        ));
