    ...
    int phaseCount(String phase) {
      return patients.where((p) {
        ...
        switch (phase) {
          ...
          case 'tratamiento':
            return s == TreatmentStage.instalacion ||
                s == TreatmentStage.controles;
          case 'retencion':
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${activePatients.length} pacientes en programa · actualizado hoy',
                  style: const TextStyle(
                ...
                Row(
                  children: [
                    Expanded(
                      child: _MiniStageCard(
                        value: '${phaseCount('registrado')}',
                        label: 'Registrado',
                        bg: const Color(0xFFF6EFE7),
                      ),
                    ),
                    ...
                    Expanded(
                      child: _MiniStageCard(
                        value: '${phaseCount('evaluacion')}',
                        label: 'Evaluación',
                        bg: const Color(0xFFEFE7F7),
                      ),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: _MiniStageCard(
                        value: '${phaseCount('tratamiento')}',
                        label: 'Tratamiento',
                        bg: const Color(0xFFE5F3EA),
                      ),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: _MiniStageCard(
                        value: '${phaseCount('retencion')}',
                        label: 'Retención',
                        bg: const Color(0xFFFFF4D8),
                      ),
                    ),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      _TypeRow(
                        type: TreatmentType.convencional,
                        count: byType[TreatmentType.convencional] ?? 0,
                        max: patients.length,
                      ),
                      _TypeRow(
                        type: TreatmentType.estetico,
                        count: byType[TreatmentType.estetico] ?? 0,
                        max: patients.length,
                      ),
                      _TypeRow(
                        type: TreatmentType.autoligado,
                        count: byType[TreatmentType.autoligado] ?? 0,
                        max: patients.length,
                      ),
                      _TypeRow(
                        type: TreatmentType.alineadores,
                        count: byType[TreatmentType.alineadores] ?? 0,
                        max: patients.length,
                      ),
                      _TypeRow(
                        type: TreatmentType.ortopedia,
                        count: byType[TreatmentType.ortopedia] ?? 0,
                        max: patients.length,
                      ),
                      _TypeRow(
                        type: TreatmentType.retenedores,
                        count: byType[TreatmentType.retenedores] ?? 0,
                        max: patients.length,
                        isLast: true,
                      ),
                    ],
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        border: isLast
            ? null
            : const Border(bottom: BorderSide(color: Color(0xFFF0E3D8))),
      ),
      child: Row(
        children: [
          Container(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: critical
              ? OcgColors.error.withOpacity(0.4)
